...
class MortgageService:
    ...
    def analyze(
        ...
        # Calculate remaining interest using amortization
        total_paid = monthly_payment * term_months
        total_original_interest = total_paid - original_principal
    ...
    def calculate_refinance(
        ...
        # Estimate closing costs (typically 2-5% of loan)
        closing_cost_percent = 0.03
        closing_costs = current_principal * closing_cost_percent
        
        scenarios = []
        
        for rate in new_rate_options:
            for term in term_options:
                # Calculate new monthly payment using standard amortization formula
        ...
        # Find best scenario
        best = max(scenarios, key=lambda x: x.get("total_interest_savings", 0) if isinstance(x.get("total_interest_savings"), (int, float)) else 0, default=None)
    ...
    def _calculate_remaining_interest(
        self,
        principal: float,
        annual_rate: float,
        monthly_payment: float,
    ...
    def _estimate_refi_savings(self, current_rate: float, remaining_months: int, equity: float) -> float:
        ...
        if monthly_rate_current > monthly_rate_new:
            current_payment = 0  # Already know monthly payment
            new_payment_factor = (monthly_rate_new * (1 + monthly_rate_new)**remaining_months) / ((1 + monthly_rate_new)**remaining_months - 1)
            return round(remaining_months * 200)  # Rough estimate
        return 0